...
| 2. Catalog & search | `2026-07-11-phase-2-catalog-search.md` (ready) | Product/bundle/alias admin CRUD, Drive service-account client + verify-access, preview generation from Drive PDFs (outbox job), PLP with capsule filters, PDP with previews + bundle upsell, academic search (parser + pg_trgm) + search logging | §4 (preview gen, verify), §7, §8 (PLP/PDP), part of §9 |
| 3. Checkout & fulfillment | `2026-07-11-phase-3-checkout-fulfillment.md` (ready) | Cart, PaymentProvider interface, Razorpay gateway mode + webhook (signature, dedupe), manual UPI mode + admin confirm, fulfillment jobs (Drive grant, delivery email via Resend), entitlements, student dashboard, refund-with-revoke | §4 (grants/revocation), §5, §8 (checkout/dashboard) |
